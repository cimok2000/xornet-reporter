      ...
      if (file.startsWith("speedtest")) {
        ...
        netsh_output.stdout.on("data", (progress) => {
          ...
          if (progress.type == "download" || progress.type == "upload") {
            ...
            clearLastLine();
            console.log("[SPEEDTEST]".bgYellow.black + ` Performing: ${progress.type.yellow}` + ` Progress: ${(progress[progress.type].progress * 100).toFixed(2).toString().yellow}%` + ` Ping: ${progress.ping.jitter.toFixed(2).toString().yellow}ms`);
          }
        });
